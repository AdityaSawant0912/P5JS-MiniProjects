...
class eNFA {
    ...
    closure(a) {
        ...
        if (isOperand(a)) {
            ...
            this.connections.push([EPSILON, start, a[0].id]);
            this.extra.push([EPSILON, a[a.length - 1].id, a[0].id]);
            let tempX = a[0].x
            let tempY = a[0].y
            ...
            this.extra.push([EPSILON, start, this.count])
            this.createState(arr, this.count++, this.count - 1, arr[arr.length - 1].x + SPACE, arr[arr.length - 1].y)
        }
        return arr;
    }
    ...
    calculateArrows(){
        ...
        for (let i = 0; i < this.extra.length; i++) {
            conn = this.extra[i];
            if(conn[2] == this.tree.length - 1){
                this.arrows.push(new Arrow(conn[0], this.tree[this.getID(this.tree, conn[1])].x, this.tree[this.getID(this.tree, conn[1])].y, this.tree[this.getID(this.tree, conn[2])].x, this.tree[this.getID(this.tree, conn[2])].y, true, true));
            }else
                this.arrows.push(new Arrow(conn[0], this.tree[this.getID(this.tree, conn[1])].x, this.tree[this.getID(this.tree, conn[1])].y, this.tree[this.getID(this.tree, conn[2])].x, this.tree[this.getID(this.tree, conn[2])].y, false, true));
        }
